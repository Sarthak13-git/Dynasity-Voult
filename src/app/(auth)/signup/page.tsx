...
export default function SignupPage() {
  ...
  const handleResend = async () => {
    ...
    if (error) {
      ...
      setResendSuccess("Verification link sent successfully! Please check your inbox.");
    }
    setResending(false);
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Check if email already exists in public.profiles to prevent duplicate signup
    ...
    if (data?.session) {
      // Assuming auto-login is active if email confirmation is disabled
      router.push("/");
      router.refresh();
    } else {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-pandora-ivory px-6 pt-20">
      ...
      >
        {success ? (
          <div className="text-center bg-white p-8 border border-pandora-cream shadow-sm rounded-lg animate-fade-in">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-pandora-ivory mb-6">
              <Mail className="h-8 w-8 text-pandora-gold animate-pulse" />
            </div>
            <h1 className="font-serif text-2.5xl font-medium text-pandora-charcoal mb-4">
              Confirm Your Email
            </h1>
            <p className="text-[14px] text-pandora-gray leading-relaxed mb-6">
              We have sent a confirmation link to <span className="font-semibold text-pandora-charcoal">{email}</span>.
            </p>
            <p className="text-[13px] text-pandora-gray-light leading-relaxed mb-8">
            ...
            <div className="space-y-4">
              <button
                ...
                disabled={resending}
                className="group flex w-full items-center justify-center gap-3 bg-pandora-charcoal py-4 text-[12px] font-semibold uppercase tracking-[0.15em] text-white transition-colors hover:bg-pandora-gold disabled:opacity-50 font-medium cursor-pointer"
              >
              ...
              <button
                ...
                className="text-[12px] text-pandora-gray-light transition-colors hover:text-pandora-gold font-medium cursor-pointer"
              >
                Entered wrong details? Edit details
              </button>

              {resendSuccess && (
                <div className="w-full bg-emerald-50 text-emerald-700 text-[12px] p-2.5 rounded border border-emerald-100 font-medium text-center">
                  {resendSuccess}
                </div>
              )}

              {resendError && (
                <div className="w-full bg-red-50 text-red-600 text-[12px] p-2.5 rounded border border-red-100 font-medium text-center">
                  {resendError}
                </div>
              )}
            </div>
          ...
          <>
            ...
            <div className="text-center">
              <h1 className="font-serif text-3xl font-bold text-pandora-charcoal">
                Join DYNASITY-VOULT
              </h1>
